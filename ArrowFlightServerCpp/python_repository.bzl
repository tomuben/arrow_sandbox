...
def _get_include_dir(binary,version,p_repository_ctx): 
    ...
    include_dir = base_include_dir+"/"+version #example /usr/include/python3.10
...
def _create_arrow_libs_symlinks(binary,p_repository_ctx):
    """
    This will create appropriate (versionless) symlinks in the pyarrow libs folder:
    libarrow.so -> /<python3_path>/site-packages/pyarrow/libarrow.so.2200
    libarrow_acero.so -> /<python3_path>/site-packages/pyarrow/libarrow_acero.so.2200
    libarrow_compute.so -> /<python3_path>/site-packages/pyarrow/libarrow_compute.so.2200
    libarrow_dataset.so -> /<python3_path>/site-packages/pyarrow/libarrow_dataset.so.2200
    libarrow_flight.so -> /<python3_path>/site-packages/pyarrow/libarrow_flight.so.2200
    """
    command_result = p_repository_ctx.execute([binary, "-c", """import pyarrow as pa; pa.create_library_symlinks()"""])
    if command_result.return_code != 0:
        fail("Could not create arrow libs symlinks, got return code %s stderr: \n %s"
            % (command_result.return_code, command_result.stderr))


def _get_arrow_include_dir(binary,p_repository_ctx):
    command_result = p_repository_ctx.execute([binary, "-c", """import pyarrow as pa; print(pa.get_include())"""])
    if command_result.return_code != 0:
        fail("Could not acquire arrow include dir, got return code %s stderr: \n %s"
            % (command_result.return_code, command_result.stderr))
    arrow_include_dir = command_result.stdout.strip("\n")
    print("python arrow_include_dir: %s"%arrow_include_dir)
    return arrow_include_dir

def _get_arrow_libs_dir(binary,p_repository_ctx):
    command_result = p_repository_ctx.execute([binary, "-c", """import pyarrow as pa; print(pa.get_library_dirs()[0])"""])
    if command_result.return_code != 0:
        fail("Could not acquire arrow lib directories, got return code %s stderr: \n %s"
            % (command_result.return_code, command_result.stderr))
    arrow_libs_dir = command_result.stdout.strip("\n")
    print("python arrow_libs_dir: %s"%arrow_libs_dir)
    return arrow_libs_dir
...
def _arrow_local_repository_impl(repository_ctx):
    ...
    #_create_arrow_libs_symlinks(binary,repository_ctx)
    arrow_include_dir =_get_arrow_include_dir(binary,repository_ctx)
    arrow_libs_dir = _get_arrow_libs_dir(binary, repository_ctx)

    repository_ctx.symlink(arrow_libs_dir, "./arrow/lib")
    repository_ctx.symlink(arrow_include_dir, "./arrow/include")

    build_file_content = """
cc_library(
    name = "arrow",
    srcs = glob(["arrow/lib/lib*.so.*"]), #e.g. libarrow_python.so.2200 . Note the final dot (.) in glob.
